...
public class Scene {
    ...
    /** The medium, which deals with rays in between surfaces */
    ...
    public void incidentRadiance(Point3 o, Vector3 dir, Color outRadiance) {
        ...
        if ( getFirstIntersection(lIntRec, ray) && 
            ...
            if ( medium != null ) {
                // Also attenuate by the medium
                ray.end = lIntRec.t;
                Color atten = new Color();
                medium.attenuation(ray, atten);
                outRadiance.scale(atten);
            }
            return;
        } 
